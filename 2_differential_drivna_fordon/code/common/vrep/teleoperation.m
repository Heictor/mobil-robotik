...
disp('Teleoperation started. Use the following keys for control:');
disp('w: Move forward');
disp('s: Move backward');
disp('a: Rotate counter-clockwise');
disp('d: Rotate clockwise');
disp('space: Stop the robot');
disp('q: Quit teleoperation');

% Define speed parameters
linearSpeed = 0.2; % m/s
angularSpeed = 0.2; % rad/s
...
while true
    % Read a key press
    key = input('Enter command (w/s/a/d/stop/q): ', 's');
    
    % Process the key press
    switch key
        case 'w' % Move forward
            leftWheelVelocity = linearSpeed / R;
            rightWheelVelocity = linearSpeed / R;
        case 's' % Move backward
            leftWheelVelocity = -linearSpeed / R;
            rightWheelVelocity = -linearSpeed / R;
        case 'a' % Rotate counter-clockwise
            leftWheelVelocity = -angularSpeed * parameters.interWheelDistance / (2 * R);
            rightWheelVelocity = angularSpeed * parameters.interWheelDistance / (2 * R);
        ...
        case 'stop' % Stop
            ...
            rightWheelVelocity = 0.0;
        case 'q' % Quit
            disp('Teleoperation ended.');
            break;
        otherwise
            disp('Invalid command. Please use w/s/a/d/stop/q.');
            continue;
    end
